...
const MODEL_INFO: Record<string, ModelInfo> = {
  ...
  'nvidia/nemotron-3-nano-30b-a3b:free': {
    description: 'NVIDIA\'s 30B sparse MoE model optimized for everyday tasks. Good balance between the 9B speed and the 120B quality.',
    who: 'Developers and users who need more depth than the 9B but faster than the 120B',
    what: 'General Q&A, writing assistance, moderate analysis, summarization',
    when: 'When 9B is too shallow but 120B latency is unacceptable',
    where: 'General-purpose assistant, mid-tier workflows',
    why: 'Fills the sweet spot between NVIDIA\'s speed and quality extremes',
    strengths: ['Better reasoning than 9B', 'Faster than 120B', 'Sparse MoE efficiency', 'Good instruction following'],
    limitations: ['Not specialized — average across most domains', 'Less capable than 120B on complex tasks'],
    advantages: ['Free access', '128K context', 'Efficient sparse architecture'],
    disadvantages: ['Jack of all trades — no standout specialty'],
    freeUsage: '20 req/min — good general-purpose rate',
    context: '128K', rpm: '20 req/min',
  },
  'nvidia/nemotron-nano-12b-v2-vl:free': {
    description: 'NVIDIA\'s vision-language model capable of processing both images and text. Unique among free models for multimodal input.',
    who: 'Developers building apps that need image understanding alongside text',
    what: 'Image description, visual Q&A, chart reading, OCR assistance, image-based tasks',
    when: 'When your workflow involves images that need to be understood or described',
    where: 'Document processing with images, visual assistants, media analysis',
    why: 'Only free vision-language model — enables image+text workflows at no cost',
    strengths: ['Multimodal — image + text', 'Unique capability on free tier', 'Good visual understanding'],
    limitations: ['Smaller context than text-only models', 'Image quality affects output', 'Slower on complex images'],
    advantages: ['Free vision capability', 'NVIDIA reliability'],
    disadvantages: ['32K context limit', 'Less capable on pure text vs dedicated text models'],
    freeUsage: '20 req/min — suitable for image-heavy pipelines',
    context: '32K', rpm: '20 req/min',
  },
  'qwen/qwen3-next-80b-a3b-instruct:free': {
    description: 'Alibaba\'s 80B MoE instruction model from the Qwen3 series. Strong multilingual capability and long document handling.',
    who: 'Multilingual teams, global businesses, researchers working across languages',
    what: 'Multilingual Q&A, translation assistance, long document analysis, general instruction tasks',
    when: 'When tasks involve non-English languages or large multilingual documents',
    where: 'Global customer support, multilingual research, cross-language content pipelines',
    why: 'Best free multilingual model — trained on diverse language corpora',
    strengths: ['Strong multilingual support (100+ languages)', 'Good at long documents', 'Efficient MoE architecture', 'Solid instruction following'],
    limitations: ['Rate limited on free tier', 'Less optimized for English-only vs English-focused models'],
    advantages: ['80B params free', '131K context', 'Alibaba multilingual training'],
    disadvantages: ['English performance lags behind Llama 70B for English-only tasks'],
    freeUsage: '20 req/min — good for multilingual batch processing',
    context: '131K', rpm: '20 req/min',
  },
  'google/gemma-4-26b-a4b-it:free': {
    description: 'Google\'s 26B sparse MoE variant from the Gemma 4 series. Faster and more efficient than the 31B while retaining most capabilities.',
    who: 'Users who want Google-quality responses with lower latency than the 31B',
    what: 'Chat, summarization, instruction following, general text tasks',
    when: 'When speed matters but you still want Google\'s quality signal',
    where: 'Interactive assistants, real-time workflows, chat applications',
    why: 'MoE efficiency gives near-31B quality at significantly faster speed',
    strengths: ['Faster than 31B', 'Good instruction following', 'Google safety alignment', 'Efficient MoE architecture'],
    limitations: ['Slightly less capable than 31B on complex tasks', 'Still somewhat conservative on edge cases'],
    advantages: ['Free Google-quality model', '128K context', 'MoE speed advantage'],
    disadvantages: ['Not as strong as 31B for deep analysis'],
    freeUsage: '20 req/min — great for interactive use',
    context: '128K', rpm: '20 req/min',
  },
  'poolside/laguna-xs.2:free': {
    description: 'Poolside\'s extra-small coding model. Built specifically for fast code completions and inline suggestions.',
    who: 'Developers wanting quick code completions without switching to a large model',
    what: 'Code completion, short function generation, simple bug fixes, inline suggestions',
    when: 'During active coding sessions where low latency is critical',
    where: 'IDE plugins, code editors, lightweight developer tooling',
    why: 'Optimized for speed in a coding context — faster than general models at code tasks',
    strengths: ['Very fast code completions', 'Low latency for coding', 'Efficient for small code tasks'],
    limitations: ['Smaller context — not for full codebase analysis', 'Less capable on large architectures'],
    advantages: ['Free coding model', 'Optimized inference speed'],
    disadvantages: ['32K context limits codebase-level tasks', 'Less capable than Laguna M.1'],
    freeUsage: '20 req/min — good for active development sessions',
    context: '32K', rpm: '20 req/min',
  },
  'poolside/laguna-m.1:free': {
    description: 'Poolside\'s medium-tier coding model. More capable than XS for larger coding tasks while staying focused on code quality.',
    who: 'Developers working on non-trivial coding tasks, debugging, or code review',
    what: 'Code generation, debugging complex issues, code review, refactoring larger files',
    when: 'When XS is insufficient for the complexity of the coding task',
    where: 'Code review tools, development assistants, refactoring pipelines',
    why: 'Stronger code understanding than XS while still faster than general 70B models',
    strengths: ['Better than XS on complex code', 'Code-focused training', 'Good debugging capability'],
    limitations: ['May lag on very large or complex architectures', 'Limited context for monorepo tasks'],
    advantages: ['Free coding-specialized model', 'Stronger than XS'],
    disadvantages: ['32K context — not for whole-codebase analysis'],
    freeUsage: '20 req/min — solid for regular development work',
    context: '32K', rpm: '20 req/min',
  },
  'liquid/lfm-2.5-1.2b-thinking:free': {
    description: 'Liquid AI\'s tiny thinking-enabled model. Applies lightweight chain-of-thought reasoning to simple problems at ultra-low latency.',
    who: 'Developers needing a fast model with basic reasoning for simple automated pipelines',
    what: 'Simple logic, classification with reasoning, lightweight chain-of-thought tasks',
    when: 'When you need some reasoning transparency but can\'t afford a large model\'s latency',
    where: 'Automated decision pipelines, simple agent steps, classification with explanation',
    why: 'Adds reasoning capability at minimal cost — unique for a 1.2B model',
    strengths: ['Reasoning steps visible', 'Ultra-fast for its capability', 'Low resource usage'],
    limitations: ['1.2B — reasoning depth is very limited', 'Not suitable for complex multi-step problems'],
    advantages: ['30 req/min free', 'Thinking capability at tiny model size'],
    disadvantages: ['Shallow reasoning — may fail on anything non-trivial'],
    freeUsage: '30 req/min — use for simple reasoned classification at scale',
    context: '32K', rpm: '30 req/min',
  },
  'z-ai/glm-4.5-air:free': {
    description: 'Zhipu AI\'s GLM-4.5 Air model — strong Chinese-English bilingual model optimized for efficiency and cross-lingual tasks.',
    who: 'Chinese-speaking users, businesses serving Chinese markets, cross-lingual researchers',
    what: 'Chinese-English translation, bilingual Q&A, cross-cultural content, Chinese document analysis',
    when: 'When tasks involve Chinese language content or Chinese-English bilingual workflows',
    where: 'Chinese market applications, localization pipelines, bilingual content tools',
    why: 'Native Chinese training gives it an edge over Western models on Chinese tasks',
    strengths: ['Native Chinese language capability', 'Strong bilingual performance', 'Good at cross-lingual tasks', 'Cultural context awareness'],
    limitations: ['Less optimized for English-only tasks vs English-native models', 'Smaller community and fewer benchmarks'],
    advantages: ['Free Chinese-capable model', '128K context', 'Cultural nuance awareness'],
    disadvantages: ['English performance behind Llama/GPT equivalents for pure English tasks'],
    freeUsage: '20 req/min — good for bilingual pipelines',
    context: '128K', rpm: '20 req/min',
  },
  'cognitivecomputations/dolphin-mistral-24b-venice-edition:free': {
    description: 'An uncensored fine-tune of Mistral 24B by Cognitive Computations, hosted via Venice with enhanced privacy. Designed for unrestricted creative and research use.',
    who: 'Researchers, creative writers, and developers needing fewer content restrictions',
    what: 'Creative fiction, roleplay, uncensored Q&A, research into sensitive topics',
    when: 'When other models refuse legitimate creative or research requests',
    where: 'Creative writing platforms, adult fiction, research applications',
    why: 'Removes the conservative alignment of standard models for legitimate unrestricted use',
    strengths: ['Fewer content restrictions', 'Good creative writing', 'Privacy-focused via Venice', 'Strong instruction following'],
    limitations: ['May produce unfiltered or explicit content', 'Not suitable for customer-facing applications', 'Venice provider rate limits'],
    advantages: ['Free uncensored model', 'Venice privacy guarantee', '32K context'],
    disadvantages: ['Not appropriate for general user-facing deployments', 'Upstream throttling from Venice'],
    freeUsage: '20 req/min — use responsibly, not for public-facing products',
    context: '32K', rpm: '20 req/min',
  },
  'openrouter/owl-alpha': {
    description: 'OpenRouter\'s experimental alpha model — an early-access routing and reasoning model under active development.',
    who: 'Developers interested in testing cutting-edge OpenRouter capabilities',
    what: 'Experimental tasks, testing new routing behaviours, early feature access',
    when: 'When evaluating new OpenRouter capabilities before general availability',
    where: 'Development and testing environments only',
    why: 'Early access to OpenRouter\'s own model development',
    strengths: ['Early feature access', 'OpenRouter-native integration'],
    limitations: ['Alpha — may be unstable or unavailable', 'No SLA or consistency guarantees', 'API behaviour may change without notice'],
    advantages: ['Free access to experimental features'],
    disadvantages: ['Not production-ready', 'Can break or behave unexpectedly'],
    freeUsage: 'Varies — alpha tier, no guarantees',
    context: 'Varies', rpm: 'Varies',
  },
}
